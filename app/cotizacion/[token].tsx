  ...
  return (
    <ScrollView contentContainerStyle={styles.page}>
      ...
      {quote ? <View style={styles.card}>
        ...
        {currentOrder ? <View style={reservationActive || paymentConfirmed ? styles.success : styles.inactive}><MaterialIcons name={paymentConfirmed ? 'verified' : reservationActive ? 'check-circle' : 'schedule'} size={34} color={paymentConfirmed || reservationActive ? '#25835b' : COLORS.textSecondary} /><View style={styles.successCopy}><Text style={styles.successTitle}>{paymentConfirmed ? 'Pago confirmado' : reservationActive ? 'Productos reservados' : 'La reserva ya no está activa'}</Text><Text style={styles.muted}>{paymentConfirmed ? 'La compra fue confirmada por Hogar Conectado.' : reservationActive ? 'La reserva dura 24 horas. El pago todavía está pendiente.' : 'Contactá al vendedor para solicitar una nueva cotización.'}</Text>{reservationActive && currentOrder ? <Text style={styles.expiry}>Vence: {new Date(currentOrder.reservaVenceAt).toLocaleString('es-AR')}</Text> : null}</View></View> : <>
