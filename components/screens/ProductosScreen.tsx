...
📱 Modelo: ${producto.modelo}`;
  ...
  const renderProducto = ({ item, index = 0 }: { item: Producto; index?: number }) => {
    ...
    const storefrontCardWidth = !isWideScreen
      ? (width >= 350 ? (Platform.OS === "web" ? "calc(50% - 6px)" : "48%") : "100%")
      : getCardWidth();

    return (
      <View
        key={item._id}
        style={[styles.cardContainer, { width: (isStorefront ? storefrontCardWidth : getCardWidth()) as any }]}
      >
        {isStorefront ? (
          <CatalogProductCard
            producto={productoConPrecios}
            imagePriority={index < 4 ? "high" : index < 8 ? "normal" : "low"}
            selected={consultProducts.some(product => product._id === item._id)}
            onPress={() => {
              setSelectedProduct(item);
              setDetailModalVisible(true);
            }}
            onSelect={() => setConsultProducts(current => (
    ...
    <>
      {isWeb && isWideScreen ? (
        ...
        <View style={styles.mobileLayout}>
          ...
          >
            <View style={[styles.mobileContentWithBackground, isStorefront && styles.storefrontContent]}>
              {isStorefront ? (
                <View style={styles.storefrontIntro}>
                  <ThemedText style={styles.storefrontIntroEyebrow}>TU VIDRIERA, EN UN SOLO LUGAR</ThemedText>
                  <ThemedText style={styles.storefrontIntroTitle}>Encontrá lo que necesitás</ThemedText>
                  <ThemedText style={styles.storefrontIntroSubtitle}>Elegí uno o varios productos y consultanos sin compromiso.</ThemedText>
              ...
              <View style={[styles.searchContainer, isStorefront ? styles.storefrontSearch : styles.standardMobileSearch]}>
                <MaterialIcons
                  name="search"
                  size={20}
              ...
              {isStorefront ? (
                <TouchableOpacity
                  accessibilityRole="button"
                  accessibilityLabel="Abrir filtros de productos"
                  style={styles.storefrontFilterButton}
                  onPress={() => setFiltersModalVisible(true)}
                >
              ...
              {isStorefront ? (
                ...
                >
                  <TouchableOpacity
                    ...
                    accessibilityState={{ selected: !filtroCategoria }}
                    onPress={() => handleCategoriaChange("")}
                    style={[styles.categoryPill, !filtroCategoria && styles.categoryPillActive]}
                  >
                    <ThemedText style={[styles.categoryPillText, !filtroCategoria && styles.categoryPillTextActive]}>Todos</ThemedText>
                  </TouchableOpacity>
                  {categorias.map(category => (
                    <TouchableOpacity
                      key={category._id}
                      accessibilityRole="tab"
                      accessibilityState={{ selected: filtroCategoria === category._id }}
                      onPress={() => handleCategoriaChange(category._id)}
                      style={[styles.categoryPill, filtroCategoria === category._id && styles.categoryPillActive]}
                    >
                      <ThemedText numberOfLines={1} style={[styles.categoryPillText, filtroCategoria === category._id && styles.categoryPillTextActive]}>
                        {category.nombre.replace(/^Essen · /, "")}
                      </ThemedText>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
              ...
              <FadeInView delay={productosLoading ? 0 : 200}>
                <ThemedView style={styles.productListContainer}>
                  {productosLoading && productosFiltrados.length > 0 ? <LoadingBar label="Actualizando catálogo…" /> : null}
                  {productosFiltrados.length === 0 ? (
                    productosLoading ? <ProductCatalogSkeleton /> : <DataStatePanel {...catalogState} />
                  ) : (
                    <View style={[styles.mobileList, isStorefront && styles.storefrontGrid]}>
                      {productosFiltrados.map((producto, index) =>
                        renderProducto({ item: producto, index })
                      )}
                    </View>
                ...
                >
                  <View style={styles.detailContainer}>
                    ...
                    <View style={styles.detailSection}>
                      {isStorefront ? <ThemedText style={styles.storefrontDetailCategory}>
                        {typeof selectedProduct.categoria === "string" ? selectedProduct.categoria : selectedProduct.categoria.nombre}
                      </ThemedText> : null}
                      <ThemedText style={[styles.detailTitle, isStorefront && styles.storefrontDetailTitle]}>
                      ...
                      {!isStorefront && <View style={styles.detailRow}>
                      ...
                      {selectedProduct.descripcion && !isStorefront && (
                      ...
                      {selectedProduct.descripcion && isStorefront ? (
                        <ThemedText style={styles.storefrontDetailDescription}>{selectedProduct.descripcion}</ThemedText>
                      ) : null}

                      {isStorefront ? (
                        <View style={styles.storefrontDetailPriceBlock}>
                          <ThemedText style={styles.storefrontDetailPriceLabel}>Precio contado</ThemedText>
                          <ThemedText style={styles.storefrontDetailPrice}>
                            ${formatPrecioLocal(convertirAProductoConPrecios(selectedProduct).precios.contado)}
                          </ThemedText>
                        </View>
                      ) : null}
                      ...
                      <View style={[styles.detailRow, isStorefront && styles.storefrontAvailabilityRow]}>
                        <ThemedText style={styles.detailLabel}>
                          Stock:
                        </ThemedText>
                        <ThemedText
                          style={[
                            ...
                            isStorefront && styles.storefrontAvailabilityValue,
                          ]}
                        >
                          {isStorefront
                            ? (selectedProduct.stock.disponible && selectedProduct.stock.cantidad > 0 ? "Disponible" : "Disponible a pedido")
                            : `${selectedProduct.stock.cantidad} unidades${selectedProduct.stock.disponible ? " ✅ Disponible" : " ❌ No disponible"}`}
                        </ThemedText>
                      </View>
                    </View>
                    ...
                    <View style={styles.detailActionsContainer}>
                      {isStorefront ? <TouchableOpacity
                        style={[styles.detailEditButton, styles.storefrontDetailAction]}
                        onPress={() => {
                          setConsultProducts(current => current.some(product => product._id === selectedProduct._id)
                            ? current
                        ...
                        accessibilityRole="button"
                      >
                        <MaterialIcons name="favorite-border" size={20} color={COLORS.ink} />
                        <ThemedText style={styles.storefrontDetailActionText}>¡Lo quiero!</ThemedText>
...
const styles = StyleSheet.create({
  ...
  storefrontJoinLink: { alignSelf: 'flex-start', minHeight: 40, flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: SPACING.sm },
  storefrontJoinLinkText: { color: COLORS.primaryDark, fontSize: 13, fontWeight: '800' },
  mobileHeaderActions: {
    alignItems: "flex-end",
    marginBottom: SPACING.md,
  },
  ...
  storefrontDetailPriceBlock: {
    marginTop: SPACING.md,
    padding: SPACING.md,
    borderRadius: RADIUS.md,
    backgroundColor: COLORS.cardBackground,
  },
  storefrontDetailPriceLabel: {
    color: COLORS.textSecondary,
    fontSize: 12,
    fontWeight: "700",
    textTransform: "uppercase",
  },
  storefrontDetailPrice: {
    marginTop: 2,
    color: COLORS.primaryDark,
    fontSize: 26,
    fontWeight: "900",
  },
  storefrontDetailCategory: {
    color: COLORS.primaryDark,
    fontSize: 12,
    fontWeight: "800",
    textTransform: "uppercase",
  ...
  storefrontDetailTitle: {
    marginBottom: SPACING.xs,
    textAlign: "left",
    color: COLORS.ink,
  },
  storefrontDetailDescription: {
    color: COLORS.textSecondary,
    fontSize: 15,
    lineHeight: 22,
  },
  ...
  storefrontAvailabilityValue: {
    flex: 0,
    textAlign: "left",
    color: COLORS.ink,
    fontSize: 14,
  },
  storefrontDetailAction: { backgroundColor: COLORS.secondary },
  storefrontDetailActionText: { color: COLORS.ink, fontWeight: "800" },
